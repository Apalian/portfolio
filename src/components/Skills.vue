...
const maxDistance = computed(() => {
  return Math.sqrt(
    Math.pow(centerRow.value, 2) + Math.pow(centerCol.value, 2)
  );
});
...
const specialHexagons = ref([
  ...
  { row: 0, col: -1, skill: 'MySql', icon: 'logos:mysql' },
  { row: 0, col: 0, skill: 'PostgreSQL', icon: 'logos:postgresql' },
  { row: 0, col: 1, skill: 'MongoDB', icon: 'devicon:mongodb' },
...
const getSpecialHexagon = (row, col) => {
  ...
  return specialHexagons.value.find(
    hex => hex.row === relativeRow && hex.col === relativeCol
  );
};
