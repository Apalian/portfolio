<template>
  <footer class="bg-gray-900 text-white py-12">
    <div class="max-w-6xl mx-auto px-6">
      <!-- Contenu principal du footer -->
      <div class="grid md:grid-cols-3 gap-8 mb-8">
        <!-- À propos -->
        <div ref="aboutRef" class="opacity-0">
          <h3 class="text-xl font-bold mb-4">Colin Lespilette</h3>
          <p class="text-gray-400 leading-relaxed">
            {{ t('footer.description') }}
          </p>
        </div>

        <!-- Liens rapides -->
        <div ref="linksRef" class="opacity-0">
          <h3 class="text-xl font-bold mb-4">{{ t('footer.quickLinks') }}</h3>
          <ul class="space-y-2">
            <li>
              <a
                href="#hero"
                @click.prevent="scrollToSection('hero')"
                class="text-gray-400 hover:text-white transition-colors duration-300"
              >
                {{ t('nav.home') }}
              </a>
            </li>
            <li>
              <a
                href="#about"
                @click.prevent="scrollToSection('about')"
                class="text-gray-400 hover:text-white transition-colors duration-300"
              >
                {{ t('nav.about') }}
              </a>
            </li>
            <li>
              <a
                href="#education"
                @click.prevent="scrollToSection('education')"
                class="text-gray-400 hover:text-white transition-colors duration-300"
              >
                {{ t('nav.education') }}
              </a>
            </li>
            <li>
              <a
                href="#skills"
                @click.prevent="scrollToSection('skills')"
                class="text-gray-400 hover:text-white transition-colors duration-300"
              >
                {{ t('nav.skills') }}
              </a>
            </li>
            <li>
              <a
                href="#projects"
                @click.prevent="scrollToSection('projects')"
                class="text-gray-400 hover:text-white transition-colors duration-300"
              >
                {{ t('nav.projects') }}
              </a>
            </li>
            <li>
              <a
                href="#contact"
                @click.prevent="scrollToSection('contact')"
                class="text-gray-400 hover:text-white transition-colors duration-300"
              >
                {{ t('nav.contact') }}
              </a>
            </li>
          </ul>
        </div>

        <!-- Réseaux sociaux -->
        <div ref="socialRef" class="opacity-0">
          <h3 class="text-xl font-bold mb-4">{{ t('footer.followMe') }}</h3>
          <div class="flex space-x-4">
            <!-- LinkedIn -->
            <a
              href="https://linkedin.com/in/colin-lespilette"
              target="_blank"
              rel="noopener noreferrer"
              class="w-12 h-12 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-linear-to-br hover:from-kelly-green hover:to-acid-green transition-all duration-300 transform hover:scale-110"
            >
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
              </svg>
            </a>

            <!-- GitHub -->
            <a
              href="https://github.com/colin-lespilette"
              target="_blank"
              rel="noopener noreferrer"
              class="w-12 h-12 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-linear-to-br hover:from-kelly-green hover:to-acid-green transition-all duration-300 transform hover:scale-110"
            >
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
              </svg>
            </a>

            <!-- Email -->
            <a
              href="mailto:colin.lespilette@example.com"
              class="w-12 h-12 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-linear-to-br hover:from-kelly-green hover:to-acid-green transition-all duration-300 transform hover:scale-110"
            >
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
              </svg>
            </a>
          </div>
        </div>
      </div>

      <!-- Séparateur -->
      <div class="border-t border-gray-800 mb-6"></div>

      <!-- Copyright -->
      <div ref="copyrightRef" class="text-center text-gray-400 opacity-0">
        <p>
          © {{ currentYear }} Colin Lespilette. {{ t('footer.rights') }}
        </p>
      </div>
    </div>
  </footer>
</template>

<script setup lang="ts">
import { ref, computed, onMounted, onUnmounted } from 'vue'
import { gsap } from 'gsap'
import { ScrollToPlugin } from 'gsap/ScrollToPlugin'
import { useI18n } from 'vue-i18n'

gsap.registerPlugin(ScrollToPlugin)

const { t } = useI18n()

// Refs
const aboutRef = ref<HTMLElement>()
const linksRef = ref<HTMLElement>()
const socialRef = ref<HTMLElement>()
const copyrightRef = ref<HTMLElement>()

let observer: IntersectionObserver

// Année actuelle
const currentYear = computed(() => new Date().getFullYear())

// Méthode de scroll
const scrollToSection = (sectionId: string) => {
  const targetElement = document.getElementById(sectionId)

  if (!targetElement) {
    console.error(`Target element '${sectionId}' not found`)
    return
  }

  gsap.to(window, {
    duration: 1.5,
    scrollTo: {
      y: targetElement,
      offsetY: 0
    },
    ease: "power2.inOut"
  })
}

// Animations
const animateElement = (element: Element, delay: number = 0) => {
  gsap.fromTo(
    element,
    { opacity: 0, y: 30 },
    {
      opacity: 1,
      y: 0,
      duration: 0.8,
      delay: delay,
      ease: 'power2.out',
    }
  )
}

// Lifecycle
onMounted(() => {
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          if (entry.target === aboutRef.value) {
            animateElement(entry.target, 0)
          } else if (entry.target === linksRef.value) {
            animateElement(entry.target, 0.2)
          } else if (entry.target === socialRef.value) {
            animateElement(entry.target, 0.4)
          } else if (entry.target === copyrightRef.value) {
            animateElement(entry.target, 0.6)
          }
          observer.unobserve(entry.target)
        }
      })
    },
    {
      threshold: 0.2,
    }
  )

  const elements = [aboutRef.value, linksRef.value, socialRef.value, copyrightRef.value]
  elements.forEach((el) => {
    if (el) observer.observe(el)
  })
})

onUnmounted(() => {
  if (observer) observer.disconnect()
})
</script>

<style scoped>
/* Smooth hover effects */
a {
  position: relative;
}

a::after {
  content: '';
  position: absolute;
  width: 0;
  height: 2px;
  bottom: -2px;
  left: 0;
  background: linear-gradient(to right, #00FF00, #ADFF2F);
  transition: width 0.3s ease;
}

a:hover::after {
  width: 100%;
}
</style>
